...
void Trainer::train(const std::vector<Tensor>& inputs, const std::vector<Tensor>& targets) {
  train_epochs(inputs, targets);
}

float Trainer::train_epochs(const std::vector<Tensor>& inputs, const std::vector<Tensor>& targets) {
  float final_loss = 0.0f;
  for (size_t epoch = 0; epoch < m_config.epochs; ++epoch) {
    final_loss = train_epoch(inputs, targets);
  }
  return final_loss;
}
