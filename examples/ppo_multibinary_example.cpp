...
int main() {
  ...
  if (!agent) {
    std::cout << "Failed to create multi-binary PPO agent\n";
    nn_free_model(actor);
  ...
  std::vector<float> reward(1, 0.25f);
  ...
  for (size_t step = 0; step < rollout_steps; ++step) {
    ...
    std::cout << "step " << step << " actions=[" << actions[0] << ", " << actions[1] << ", " << actions[2] << "]"
